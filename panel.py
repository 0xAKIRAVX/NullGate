...
Railway Xray panel (dual-core)
  ...
  xray               -> listens on 127.0.0.1 for the HTTP transports + 0.0.0.0:TCP_APP_PORT for Reality
                        + 0.0.0.0:TCP2_APP_PORT for Reality-gRPC (2nd TCP proxy)
  sing-box (opt.)    -> AnyTLS + ShadowTLS(3) on TCP ports, Hysteria2/TUIC on UDP (VPS only)
...
# Protocol keys: built-in inbounds + the Reality TCP link + WireGuard + the
# dual-core additions (Reality-gRPC on Xray; AnyTLS / ShadowTLS / Hysteria2 /
# TUIC on sing-box). Each can be turned off globally (Settings) or per-user
# (empty override = follow globals).
PROTO_KEYS = ("vless-ws", "vless-xhttp", "vless-hu", "vmess-ws", "trojan-ws", "vless-reality",
              "vless-reality-grpc", "anytls", "shadowtls", "hysteria2", "tuic", "wireguard")
PROTO_LABELS = {
    ...
    "vless-reality-grpc": "VLESS · Reality (gRPC)", "anytls": "AnyTLS",
    "shadowtls": "ShadowTLS v3 + SS2022", "hysteria2": "Hysteria2 (UDP)",
    "tuic": "TUIC v5 (UDP)", "wireguard": "WireGuard (UDP)",
}
# protocols that run on the sing-box core
SB_KEYS = ("anytls", "shadowtls", "hysteria2", "tuic")
# UDP-only protocols: unreachable on Railway (TCP/HTTPS edge), usable on a VPS
UDP_KEYS = ("hysteria2", "tuic", "wireguard")
...
# 2nd TCP proxy: VLESS+Reality over gRPC (Xray). Same Reality identity, new transport.
TCP2_APP_PORT = env_int("RAILWAY_TCP2_APPLICATION_PORT", env_int("TCP2_APP_PORT", 9001))
TCP2_HOST = env("TCP2_HOST") or env("RAILWAY_TCP2_PROXY_DOMAIN")
TCP2_PUBLIC_PORT = env("TCP2_PORT") or env("RAILWAY_TCP2_PROXY_PORT")
GRPC_SERVICE = re.sub(r"[^A-Za-z0-9_-]", "", env("GRPC_SERVICE") or "nullgate-grpc")[:40] or "nullgate-grpc"
# sing-box TCP inbounds. On Railway each needs its own TCP Proxy; on a VPS the
# ports are simply opened in the firewall.
ANYTLS_APP_PORT = env_int("ANYTLS_APP_PORT", 8443)
ANYTLS_HOST = env("ANYTLS_HOST")
ANYTLS_PUBLIC_PORT = env("ANYTLS_PORT")
STLS_APP_PORT = env_int("STLS_APP_PORT", 8444)
STLS_HOST = env("STLS_HOST")
STLS_PUBLIC_PORT = env("STLS_PORT")
STLS_SNI = env("STLS_SNI") or "www.cloudflare.com"  # ShadowTLS handshake target
# WireGuard is UDP-only: it cannot pass through Railway's HTTP/TCP proxies, but
# works out of the box on a personal VPS (open UDP WG_PORT in the firewall).
WG_PORT = env_int("WG_PORT", 51820)
# sing-box UDP inbounds (VPS only - Railway has no public UDP)
HY2_PORT = env_int("HY2_PORT", 8445)
TUIC_PORT = env_int("TUIC_PORT", 8446)
SINGBOX_BIN = env("SINGBOX_BIN") or "sing-box"
SINGBOX_VERSION = env("SINGBOX_VERSION") or "1.12.9"  # pinned for auto-provision
SINGBOX_AUTO = env("SINGBOX_AUTO") != "0"             # auto-download when missing
API_PORT = 10085  # Xray stats API (127.0.0.1 only)
SB_CONF = "/tmp/singbox.json"
SB_LOG = "/tmp/singbox.log"
BUILTIN_PORTS = {PORT, 10003, TCP_APP_PORT, TCP2_APP_PORT, API_PORT,
                 10001, 10002, 10004, 10005, 10006,
                 ANYTLS_APP_PORT, STLS_APP_PORT, HY2_PORT, TUIC_PORT}

# Railway auto-detection: UDP protocols cannot be exposed there, so they stay
# dormant (config/link generation skipped) unless FORCE_UDP=1 is set (VPS).
IS_RAILWAY = bool(os.environ.get("RAILWAY_ENVIRONMENT") or os.environ.get("RAILWAY_PROJECT_ID")
                  or os.environ.get("RAILWAY_PUBLIC_DOMAIN"))


def udp_ok():
    return env("FORCE_UDP") == "1" or not IS_RAILWAY
...
# ───────────────────────── WireGuard keys (RFC 7748 X25519, pure python) ─────────────────────────
# Same curve family as Reality, but WireGuard needs plain Curve25519 keys.
# Derived deterministically from SECRET so they survive redeploys without being
# stored - exactly like the Reality seed above.
WG = {"priv": None, "pub": None}
_WG_P = 2 ** 255 - 19


def _clamp(k: bytes) -> bytes:
    """WireGuard-canonical private key (RFC 7748 decodeScalar25519)."""
    b = bytearray(k)
    b[0] &= 248
    b[31] &= 127
    b[31] |= 64
...
def _x25519(k_bytes: bytes) -> int:
    """X25519 scalar * base point (9) - Montgomery ladder, RFC 7748 §5."""
    a = int.from_bytes(_clamp(k_bytes), "little")
    x1 = 9
    ...
    for t in reversed(range(255)):
        ...
        z3 = z3 * x1 % _WG_P
        x2 = aa * bb % _WG_P
        z2 = e * (aa + 121665 * e) % _WG_P
    if swap:
        x2, x3 = x3, x2
        z2, z3 = z3, z2
    return x2 * pow(z2, _WG_P - 2, _WG_P) % _WG_P


def _b64_int(v: int) -> str:
    return base64.b64encode(v.to_bytes(32, "little")).decode()


def init_wg():
    seed = hashlib.sha256((SECRET + ":wg-server").encode()).digest()
    priv = _clamp(seed)
    WG["priv"] = base64.b64encode(priv).decode()
    WG["pub"] = _b64_int(_x25519(priv))


def wg_user_keys(c):
    """(private, public) base64 keypair for one user - stable across redeploys."""
    seed = hashlib.sha256((SECRET + ":wg:" + c["id"]).encode()).digest()
    priv = _clamp(seed)
    return base64.b64encode(priv).decode(), _b64_int(_x25519(priv))


def get_wg_ip(c):
    """Allocate a stable 10.8.x.y tunnel address for the user (persisted)."""
    with state_lock:
        if c.get("wg_ip"):
            return c["wg_ip"]
        used = {x.get("wg_ip") for x in state["clients"] if x.get("wg_ip")}
        for o3 in range(256):
...
def wireguard_conf(c, host):
    """Ready-to-import WireGuard client .conf, or (None, error)."""
    if not WG["priv"]:
        return None, "کلید WireGuard آماده نیست"
    if not proto_enabled("wireguard"):
        return None, "WireGuard در تنظیمات خاموش است"
    if not udp_ok():
        return None, "این سرور UDP عمومی ندارد (Railway) - برای WireGuard به VPS نیاز است"
    if "wireguard" not in user_protocols(c):
        return None, "WireGuard برای این کاربر فعال نیست"
    ip = get_wg_ip(c)
    if not ip:
        return None, "ظرفیت آدرس‌های تونل پر شده است"
    with state_lock:
        addr = state["address"] or host
    priv, _ = wg_user_keys(c)
    return (
        "[Interface]\n"
        f"PrivateKey = {priv}\n"
        f"Address = {ip}/32\n"
        ...
        f"Endpoint = {addr}:{WG_PORT}\n"
        "PersistentKeepalive = 25\n"
    ), None


# ───────────────────────── sing-box core (2nd core) ─────────────────────────
# Xray covers the HTTP-family transports; sing-box adds AnyTLS, ShadowTLS v3
# (+ Shadowsocks-2022 behind it), Hysteria2 and TUIC. The binary is located on
# PATH or auto-downloaded once (pinned release) - without it, the panel still
# runs and only these protocols are marked unavailable.
SB = {"path": None, "ver": "", "note": "", "checked": False}
sb_proc = None
sb_info = {"restarts": 0, "since": 0.0, "active": None}


def sb_running():
    return bool(sb_proc and sb_proc.poll() is None)
...
def _sb_version(path):
    try:
        out = subprocess.run([path, "version"], capture_output=True, text=True, timeout=15).stdout
        m = re.search(r"version (\d+\.\d+\.\d+)", out or "")
        return m.group(1) if m else ""
    except Exception:
        return ""


def _download_singbox(dest):
    """Download the pinned sing-box release for this architecture. Returns path or None."""
    arch = {"x86_64": "amd64", "amd64": "amd64", "aarch64": "arm64", "arm64": "arm64"}.get(platform.machine().lower())
    if not arch:
        SB["note"] = f"معماری {platform.machine()} پشتیبانی نمی‌شود"
        return None
    ...
    tgz = dest + ".tar.gz"
    try:
        print(f"sing-box: downloading {url} ...", flush=True)
        req = urllib.request.Request(url, headers={"User-Agent": "nullgate-panel"})
        ...
        with tarfile.open(tgz, "r:gz") as tf:
            member = next(m for m in tf.getmembers() if m.isfile() and m.name.endswith("/sing-box"))
            member.name = os.path.basename(member.name)
            tf.extract(member, os.path.dirname(dest))
        os.remove(tgz)
        bin_path = os.path.join(os.path.dirname(dest), "sing-box")
        os.chmod(bin_path, 0o755)
        v = _sb_version(bin_path)
        if not v:
            SB["note"] = "فایل دانلودشده sing-box اجرا نشد"
            return None
        print(f"sing-box: {v} ready at {bin_path}", flush=True)
        return bin_path
    except Exception as e:
        SB["note"] = f"دانلود sing-box ناموفق بود: {str(e)[:120]}"
        print("sing-box download failed:", e, flush=True)
        try:
            os.remove(tgz)
        except Exception:
            pass
...
def provision_singbox():
    """Locate (or fetch) the sing-box binary. Idempotent, runs once at startup."""
    SB["checked"] = True
    for p in (SINGBOX_BIN, "/usr/local/bin/sing-box", "/usr/bin/sing-box",
              os.path.join(APP_DIR, "sing-box"), "/tmp/ng-sb/sing-box", "/tmp/sing-box"):
        v = _sb_version(p)
        if v:
            SB["path"], SB["ver"] = p, v
            print(f"sing-box: {v} found at {p}", flush=True)
            return
    if not SINGBOX_AUTO:
        SB["note"] = SB["note"] or "sing-box نصب نیست و دانلود خودکار خاموش است (SINGBOX_AUTO=0)"
        return
    dest_dir = "/tmp/ng-sb"
    try:
        ...
        dest_dir = "/tmp"
    p = _download_singbox(os.path.join(dest_dir, "sing-box"))
    if p:
        SB["path"], SB["ver"] = p, _sb_version(p)


def ensure_certs():
    """Self-signed EC certificate for sing-box TLS inbounds (AnyTLS/Hysteria2/TUIC).
    ShadowTLS needs none (it relays a real handshake). Returns (crt, key) or (None, None)."""
    crt, key = os.path.join(CERT_DIR, "ng.crt"), os.path.join(CERT_DIR, "ng.key")
    if os.path.exists(crt) and os.path.exists(key):
        return crt, key
    with state_lock:
        sni = state.get("reality_sni") or "nullgate.local"
    try:
        os.makedirs(CERT_DIR, exist_ok=True)
        r = subprocess.run(["openssl", "req", "-x509", "-newkey", "ec",
                            "-pkeyopt", "ec_paramgen_curve:prime256v1",
                            "-keyout", key, "-out", crt, "-days", "3650", "-nodes",
                            "-subj", f"/CN={sni}"], capture_output=True, text=True, timeout=30)
        if r.returncode == 0:
            return crt, key
        print("openssl cert failed:", (r.stderr or "").strip()[:200], flush=True)
    except Exception as e:
        print("openssl unavailable:", e, flush=True)
    return None, None


# deterministic per-user credentials (stable across redeploys, like WG keys)
def sb_user_pw(c, what):
    return hashlib.sha256((SECRET + f":sb:{what}:" + c["id"]).encode()).hexdigest()[:24]


def ss_user_pw(c):
    """Shadowsocks-2022 user key (iPSK): base64 of 32 raw bytes."""
    return base64.b64encode(hashlib.sha256((SECRET + ":sb:ss:" + c["id"]).encode()).digest()).decode()


def ss_server_psk():
    """SS2022 multi-user needs a fixed server PSK besides the per-user iPSKs."""
    return base64.b64encode(hashlib.sha256((SECRET + ":sb:ss-server").encode()).digest()).decode()


def sb_anytls_ready():
    return bool((ANYTLS_HOST and ANYTLS_PUBLIC_PORT) or (not IS_RAILWAY and sb_ready()))


def sb_stls_ready():
    return bool((STLS_HOST and STLS_PUBLIC_PORT) or (not IS_RAILWAY and sb_ready()))


def build_singbox_config():
    """sing-box server config for the extra protocols. (cfg, None) or (None, reason)."""
    if not sb_ready():
        return None, SB["note"] or "sing-box در دسترس نیست"
    with state_lock:
    ...
    def sb_users(key, pwfn):
        return [{"name": c["id"], "password": pwfn(c)} for c in clients if key in user_protocols(c)]

    if proto_enabled("anytls"):
        users = sb_users("anytls", lambda c: sb_user_pw(c, "anytls"))
        if users:
            if not crt:
                crt, key = ensure_certs()
            if crt:
                inb.append({"type": "anytls", "tag": "ng-anytls", "listen": "0.0.0.0",
                            "listen_port": ANYTLS_APP_PORT, "users": users,
                            "tls": {"enabled": True, "certificate_path": crt, "key_path": key}})
            else:
                errs.append("AnyTLS: گواهی TLS ساخته نشد (openssl نیست)")
    if proto_enabled("shadowtls"):
        stls_users = sb_users("shadowtls", lambda c: sb_user_pw(c, "stls"))
        if stls_users:
            # no TLS cert needed: ShadowTLS relays a real handshake from STLS_SNI.
            # Auth is per-user at the ShadowTLS layer; the SS2022 layer uses one
            # shared PSK (sing-box clients cannot send the multi-user EIH).
            inb.append({"type": "shadowsocks", "tag": "ng-ss",
                        "method": "2022-blake3-aes-256-gcm", "password": ss_server_psk()})
            inb.append({"type": "shadowtls", "tag": "ng-stls", "listen": "0.0.0.0",
                        "listen_port": STLS_APP_PORT, "version": 3, "users": stls_users,
                        "handshake": {"server": STLS_SNI, "server_port": 443}, "detour": "ng-ss"})
    if udp_ok():
        if proto_enabled("hysteria2"):
            users = sb_users("hysteria2", lambda c: sb_user_pw(c, "hy2"))
            if users:
                if not crt:
                    crt, key = ensure_certs()
                if crt:
                    # no bandwidth caps -> Hysteria2 uses BBR automatically
                    inb.append({"type": "hysteria2", "tag": "ng-hy2", "listen": "0.0.0.0",
                                "listen_port": HY2_PORT, "users": users,
                                "ignore_client_bandwidth": False,
                                "tls": {"enabled": True, "alpn": ["h3"], "certificate_path": crt, "key_path": key}})
                else:
                    errs.append("Hysteria2: گواهی TLS ساخته نشد")
        if proto_enabled("tuic"):
            users = []
            for c in clients:
                if "tuic" not in user_protocols(c):
                    continue
                try:
                    u = str(uuid.UUID(c["id"]))  # TUIC authenticates with a real UUID
                except ValueError:
                    u = str(uuid.uuid5(uuid.NAMESPACE_DNS, c["id"]))
                users.append({"name": c["id"], "uuid": u, "password": sb_user_pw(c, "tuic")})
            if users:
                if not crt:
                    crt, key = ensure_certs()
                if crt:
                    inb.append({"type": "tuic", "tag": "ng-tuic", "listen": "0.0.0.0",
                                "listen_port": TUIC_PORT, "users": users, "congestion_control": "bbr",
                                "tls": {"enabled": True, "alpn": ["h3"], "certificate_path": crt, "key_path": key}})
                else:
                    errs.append("TUIC: گواهی TLS ساخته نشد")
    else:
        errs.append("Hysteria2/TUIC غیرفعال‌اند: این سرور UDP عمومی ندارد (Railway)")
    if not inb:
        return None, "؛ ".join(errs) or "هیچ پروتکل sing-box فعالی وجود ندارد"
    cfg = {
        "log": {"level": "warn", "timestamp": True},
        "dns": {"servers": [{"type": "udp", "tag": "ng-dns", "server": "1.1.1.1"}]},
        "inbounds": inb,
        "outbounds": [{"type": "direct", "tag": "direct", "domain_strategy": "ipv4_only"}],
        # same guardrails as the Xray core: users must not reach the host's
        # private network or Railway's internal services
        "route": {"rules": [
            {"action": "reject", "domain_suffix": ["railway.internal", "localhost", "internal"]},
            {"action": "reject", "ip_is_private": True},
        ], "final": "direct", "default_domain_resolver": {"server": "ng-dns"}},
    }
    SB["err"] = "؛ ".join(errs)
    return cfg, None
...
def _spawn_singbox():
    ...
    if not sb_ready():
        return
    cfg, why = build_singbox_config()
    if not cfg:
        if sb_proc and sb_proc.poll() is None:
            sb_proc.terminate()
        sb_info["active"] = None
        sb_info["err"] = why
        return
    ...
    with open(SB_CONF, "w") as f:
        json.dump(cfg, f)
    try:
        chk = subprocess.run([SB["path"], "check", "-c", SB_CONF], capture_output=True, text=True, timeout=30)
        if chk.returncode != 0:
            print("sing-box config invalid:", (chk.stderr or "").strip()[:400], flush=True)
            sb_info["err"] = "کانفیگ sing-box نامعتبر است - به لاگ ببینید"
            return
        if sb_proc and sb_proc.poll() is None:
            sb_proc.terminate()
            try:
                ...
                sb_proc.kill()
        try:
            if os.path.exists(SB_LOG) and os.path.getsize(SB_LOG) > 512 * 1024:
                os.remove(SB_LOG)
        except Exception:
            pass
        lf = open(SB_LOG, "ab")
        sb_proc = subprocess.Popen([SB["path"], "run", "-c", SB_CONF], stdout=subprocess.DEVNULL, stderr=lf)
        sb_info["since"] = time.time()
        sb_info["restarts"] += 1
        sb_info["active"] = frozenset(c["id"] for c in active_clients())
        print("sing-box: started", flush=True)
    except Exception as e:
        print("cannot start sing-box:", e, flush=True)
        sb_proc = None
...
def build_config():
    ...
    # VLESS+Reality over gRPC on the 2nd TCP proxy - same Reality identity, a
    # different transport shape (no flow: XTLS Vision is TCP-only).
    if REALITY["priv"] and proto_enabled("vless-reality-grpc"):
        inbounds.append({
            "tag": "vless-reality-grpc", "listen": "0.0.0.0", "port": TCP2_APP_PORT, "protocol": "vless",
            "settings": {"clients": ids_for("vless-reality-grpc"), "decryption": "none"},
            "streamSettings": {
                "network": "grpc", "security": "reality",
                "grpcSettings": {"serviceName": GRPC_SERVICE, "idle_timeout": 60, "ping_timeout": 30},
                "realitySettings": {
                    "show": False, "dest": f"{sni}:443", "xver": 0, "serverNames": [sni],
                    "privateKey": REALITY["priv"], "shortIds": [REALITY["sid"]],
                },
    ...
    # WireGuard inbound (userspace implementation - no TUN/kernel module needed).
    # One peer per active user with a unique tunnel IP; `email` feeds Xray's
    # per-user traffic counters so quota/expiry enforcement works for WG too.
    # UDP-only: never created where no public UDP exists (Railway).
    if WG["priv"] and proto_enabled("wireguard") and udp_ok():
        wg_peers = []
        for c in clients:
            if "wireguard" not in user_protocols(c):
                continue
            ...
            if not ip:
                continue
            wg_peers.append({"publicKey": wg_user_keys(c)[1], "allowedIPs": [ip + "/32"],
                             "email": c["id"], "level": 0})
        if wg_peers:
            inbounds.append({
                "tag": "wg-in", "listen": "0.0.0.0", "port": WG_PORT, "protocol": "wireguard",
                "settings": {"secretKey": WG["priv"], "address": ["10.8.0.1/32"],
                             "peers": wg_peers, "mtu": 1420},
            })
    for ib in customs:
...
def restart_cores():
    """Re-apply the user set / settings to both cores."""
    restart_xray()
    restart_singbox()
...
def sync_xray():
    """Restart the cores only when the set of allowed users changed (limit hit, expiry, renewal)."""
    with state_lock:
        now_active = frozenset(c["id"] for c in active_clients())
    if now_active != xray_info.get("active") or now_active != sb_info.get("active"):
        restart_cores()
        ...
        try:
            ...
            with lock:
                if not xray_running():
                    ...
                    _spawn_xray()
                if sb_ready() and not sb_running():
                    print("sing-box exited - restarting", flush=True)
                    _spawn_singbox()
...
def build_links(c, host):
    ...
    def pub_ep(h, p, app_port):
        """Public endpoint for an optional raw-TCP service: explicit env vars win,
        otherwise on a VPS the panel address + internal port is used directly."""
        if h and p:
            return h, str(p)
        if not IS_RAILWAY and addr:
            return addr, str(app_port)
        return None

    # VLESS + Reality over gRPC (2nd TCP proxy / VPS port) - no flow on gRPC
    ep2 = pub_ep(TCP2_HOST, TCP2_PUBLIC_PORT, TCP2_APP_PORT)
    if REALITY["pub"] and ep2 and "vless-reality-grpc" in allowed:
        n7 = fmt_name(fmt, c, "VLESS-Reality-gRPC", "VLESS", "gRPC", "Reality", ep2[0])
        links.append({"key": "vless-reality-grpc", "label": "VLESS · Reality · gRPC",
                      "url": f"vless://{cid}@{ep2[0]}:{ep2[1]}?encryption=none&security=reality&sni={sni}&fp=chrome&pbk={REALITY['pub']}&sid={REALITY['sid']}&spx=%2F&type=grpc&serviceName={q(GRPC_SERVICE)}&mode=gun#{q(n7)}"})
    # sing-box protocols: only offered when the core is actually available
    if sb_ready():
        if "anytls" in allowed:
            ep = pub_ep(ANYTLS_HOST, ANYTLS_PUBLIC_PORT, ANYTLS_APP_PORT)
            if ep:
                n8 = fmt_name(fmt, c, "AnyTLS", "AnyTLS", "", "TLS", ep[0])
                links.append({"key": "anytls", "label": "AnyTLS · sing-box",
                              "url": f"anytls://{sb_user_pw(c, 'anytls')}@{ep[0]}:{ep[1]}?sni={sni}&insecure=1#{q(n8)}"})
        if "shadowtls" in allowed:
            ep = pub_ep(STLS_HOST, STLS_PUBLIC_PORT, STLS_APP_PORT)
            if ep:
                n9 = fmt_name(fmt, c, "ShadowTLS-SS2022", "SS2022", "ShadowTLS", "TLS", ep[0])
                inner = base64.b64encode(f"2022-blake3-aes-256-gcm:{ss_server_psk()}".encode()).decode().rstrip("=")
                plugin = urllib.parse.quote(f"shadow-tls;v3;password={sb_user_pw(c, 'stls')};addr={STLS_SNI}:443", safe="")
                links.append({"key": "shadowtls", "label": "ShadowTLS v3 + SS2022 · sing-box",
                              "url": f"ss://{inner}@{ep[0]}:{ep[1]}/?plugin={plugin}&sni={q(STLS_SNI)}#{q(n9)}"})
        if udp_ok() and "hysteria2" in allowed:
            ep = pub_ep(None, None, HY2_PORT)
            if ep:
                n10 = fmt_name(fmt, c, "Hysteria2", "Hysteria2", "", "UDP", ep[0])
                links.append({"key": "hysteria2", "label": "Hysteria2 (UDP) · sing-box",
                              "url": f"hy2://{sb_user_pw(c, 'hy2')}@{ep[0]}:{ep[1]}?sni={sni}&insecure=1#{q(n10)}"})
        if udp_ok() and "tuic" in allowed:
            ep = pub_ep(None, None, TUIC_PORT)
            if ep:
                try:
                    tu = str(uuid.UUID(c["id"]))
                except ValueError:
                    tu = str(uuid.uuid5(uuid.NAMESPACE_DNS, c["id"]))
                n11 = fmt_name(fmt, c, "TUIC", "TUIC", "", "UDP", ep[0])
                links.append({"key": "tuic", "label": "TUIC v5 (UDP) · sing-box",
                              "url": f"tuic://{tu}:{sb_user_pw(c, 'tuic')}@{ep[0]}:{ep[1]}?sni={sni}&congestion_control=bbr&alpn=h3&insecure=1#{q(n11)}"})
    with state_lock:
        customs = [dict(i) for i in state["inbounds"]]
    for ib in customs:
...
def client_config_json(c):
    ...
    if tcp_ready() and "vless-reality" in allowed:
        ...
        order.append("proxy")
    if REALITY["pub"] and "vless-reality-grpc" in allowed:
        h2 = TCP2_HOST or ("" if IS_RAILWAY else addr)
        p2 = int(TCP2_PUBLIC_PORT or 0) or TCP2_APP_PORT
        if h2:
            outbounds.append({
                "tag": "alt-reality-grpc", "protocol": "vless",
                "settings": {"vnext": [{"address": h2, "port": p2,
                                        "users": [{"id": cid, "encryption": "none", "flow": "", "level": 0}]}]},
...
def singbox_client_config(c, host):
    """Ready-to-import sing-box CLIENT config for one user: every protocol the
    user has enabled (including the Xray ones - sing-box understands them all,
    except XHTTP which is Xray-only). Used by the official sing-box app."""
    if not sb_ready():
        return {"error": SB["note"] or "sing-box در دسترس نیست"}
    with state_lock:
        addr = state["address"] or host
        sni = state["reality_sni"]
        fmt = state.get("cfg_fmt") or "{name}-{label}"
    allowed = user_protocols(c)
    cid = c["id"]
    name = lambda lbl: fmt_name(fmt, c, lbl, "", "", "", addr)
    outbounds = []

    def sb_tls(server_name, insecure=False, utls=True):
        """TLS client block. QUIC-based protocols (Hysteria2/TUIC) reject uTLS."""
        t = {"enabled": True, "server_name": server_name}
        if utls:
    ...
    reality_tls = {"enabled": True, "server_name": sni,
                   "utls": {"enabled": True, "fingerprint": "chrome"},
                   "reality": {"enabled": True, "public_key": REALITY["pub"], "short_id": REALITY["sid"]}}
    # Reality over TCP (Xray inbound) - sing-box client speaks it natively
    if tcp_ready() and "vless-reality" in allowed:
        outbounds.append({"type": "vless", "tag": "proxy", "server": TCP_HOST,
                          "server_port": int(TCP_PUBLIC_PORT or 443), "uuid": cid,
                          "flow": "xtls-rprx-vision", "tls": reality_tls, "packet_encoding": "xudp"})
    # Reality over gRPC
    ep2 = (TCP2_HOST and TCP2_PUBLIC_PORT and (TCP2_HOST, int(TCP2_PUBLIC_PORT))) or \
          ((addr, TCP2_APP_PORT) if (not IS_RAILWAY and addr) else None)
    if REALITY["pub"] and ep2 and "vless-reality-grpc" in allowed:
        outbounds.append({"type": "vless", "tag": "alt-reality-grpc", "server": ep2[0],
                          "server_port": ep2[1], "uuid": cid, "tls": reality_tls,
                          "transport": {"type": "grpc", "service_name": GRPC_SERVICE}})
    if "vless-ws" in allowed:
        outbounds.append({"type": "vless", "tag": "alt-vless-ws", "server": addr, "server_port": 443,
                          "uuid": cid, "tls": sb_tls(addr),
                          "transport": {"type": "ws", "path": WS_PATH + "?ed=2048",
                                        "headers": {"Host": addr}, "max_early_data": 2048,
                                        "early_data_header_name": "Sec-WebSocket-Protocol"}})
    if "vless-hu" in allowed:
        outbounds.append({"type": "vless", "tag": "alt-vless-hu", "server": addr, "server_port": 443,
                          "uuid": cid, "tls": sb_tls(addr),
                          "transport": {"type": "httpupgrade", "path": HU_PATH, "host": addr}})
    if "vmess-ws" in allowed:
        outbounds.append({"type": "vmess", "tag": "alt-vmess-ws", "server": addr, "server_port": 443,
                          "uuid": cid, "security": "auto", "alter_id": 0, "tls": sb_tls(addr),
                          "transport": {"type": "ws", "path": VMESS_PATH + "?ed=2048",
                                        "headers": {"Host": addr}, "max_early_data": 2048,
                                        "early_data_header_name": "Sec-WebSocket-Protocol"}})
    if "trojan-ws" in allowed:
        outbounds.append({"type": "trojan", "tag": "alt-trojan-ws", "server": addr, "server_port": 443,
                          "password": cid, "tls": sb_tls(addr),
                          "transport": {"type": "ws", "path": TROJAN_PATH + "?ed=2048",
                                        "headers": {"Host": addr}}})
    if sb_ready() and "anytls" in allowed:
        ep = (ANYTLS_HOST and ANYTLS_PUBLIC_PORT and (ANYTLS_HOST, int(ANYTLS_PUBLIC_PORT))) or \
             ((addr, ANYTLS_APP_PORT) if (not IS_RAILWAY and addr) else None)
        if ep:
            outbounds.append({"type": "anytls", "tag": "alt-anytls", "server": ep[0], "server_port": ep[1],
                              "password": sb_user_pw(c, "anytls"), "tls": sb_tls(sni, insecure=True)})
    if sb_ready() and "shadowtls" in allowed:
        ep = (STLS_HOST and STLS_PUBLIC_PORT and (STLS_HOST, int(STLS_PUBLIC_PORT))) or \
             ((addr, STLS_APP_PORT) if (not IS_RAILWAY and addr) else None)
        if ep:
            outbounds.append({"type": "shadowtls", "tag": "ng-stls-client", "server": ep[0],
                              "server_port": ep[1], "version": 3, "password": sb_user_pw(c, "stls"),
                              "tls": {"enabled": True, "server_name": STLS_SNI}})
            outbounds.append({"type": "shadowsocks", "tag": "alt-shadowtls-ss", "method": "2022-blake3-aes-256-gcm",
                              "password": ss_server_psk(), "detour": "ng-stls-client"})
    if sb_ready() and udp_ok() and "hysteria2" in allowed:
        outbounds.append({"type": "hysteria2", "tag": "alt-hy2", "server": addr, "server_port": HY2_PORT,
                          "password": sb_user_pw(c, "hy2"), "tls": sb_tls(sni, insecure=True, utls=False)})
    if sb_ready() and udp_ok() and "tuic" in allowed:
        try:
            tu = str(uuid.UUID(c["id"]))
        except ValueError:
            tu = str(uuid.uuid5(uuid.NAMESPACE_DNS, c["id"]))
        outbounds.append({"type": "tuic", "tag": "alt-tuic", "server": addr, "server_port": TUIC_PORT,
                          "uuid": tu, "password": sb_user_pw(c, "tuic"), "congestion_control": "bbr",
                          "tls": {**sb_tls(sni, insecure=True, utls=False), "alpn": ["h3"]}})
    if not outbounds:
        return {"error": "no protocol enabled for this user"}
    outbounds += [{"type": "direct", "tag": "direct", "domain_strategy": "ipv4_only"}]
    endpoints = []
    if WG["priv"] and udp_ok() and "wireguard" in allowed:
        ip = get_wg_ip(c)
        if ip:
            priv, _ = wg_user_keys(c)
            # sing-box 1.12: WireGuard moved from the outbound list to `endpoints`
            endpoints.append({"type": "wireguard", "tag": "alt-wg", "system": False,
                              "address": [ip + "/32"], "private_key": priv,
                              "peers": [{"address": addr, "port": WG_PORT,
                                         "public_key": WG["pub"],
                                         "allowed_ips": ["0.0.0.0/0", "::/0"]}], "mtu": 1420})
    return {
        "log": {"level": "warn", "timestamp": True},
        ...
        "route": {
            "rules": [
                ...
                {"domain_suffix": [".ir"], "action": "direct"},
                {"rule_set": ["geosite-ir", "geoip-ir"], "action": "direct"},
...
def health_payload():
    ...
    return {"ok": xray_running() and bool(nginx_proc and nginx_proc.poll() is None),
            ...
            "singbox": sb_running(), "latency_ms": lat, "uptime": int(time.time() - START),
    ...
    def do_GET(self):
        ...
        if path.startswith("/panel/api/"):
            ...
            if path.startswith("/panel/api/client/"):
                ...
                wg_mode = cid.endswith("/wireguard")
                sb_mode = cid.endswith("/singbox")
                if wg_mode:
                    cid = cid[: -len("/wireguard")]
                if sb_mode:
                    cid = cid[: -len("/singbox")]
                with state_lock:
                ...
                if wg_mode:
                    conf, err = wireguard_conf(c, self.host())
                    if err:
                        return self.send_json(400, {"error": err}, self.auth_headers())
                    return self.send_bytes(200, conf, "text/plain; charset=utf-8",
                                           {**self.auth_headers(),
                                            "Content-Disposition": f"attachment; filename=nullgate-{safe}.conf"})
                if sb_mode:
                    cfg = singbox_client_config(c, self.host())
                    if cfg.get("error"):
                    ...
                    return self.send_bytes(200, json.dumps(cfg, ensure_ascii=False, indent=2),
                                           ...
                                           {**self.auth_headers(),
                                            "Content-Disposition": f"attachment; filename=nullgate-{safe}-singbox.json"})
    ...
    def state_payload(self):
        ...
        for c in clients:
            ...
            out.append({"name": c["name"], "id": c["id"], "main": c["id"] == MAIN_UUID,
                        ...
                        "wg": {"on": "wireguard" in user_protocols(c), "ip": c.get("wg_ip")}})
        protocols = [
        ...
        ep2 = (TCP2_HOST and TCP2_PUBLIC_PORT) or (None if IS_RAILWAY else state["address"])
        if REALITY["pub"] and ep2:
            protocols.append({"key": "vless-reality-grpc", "name": "VLESS · Reality · gRPC",
                              "path": f"{ep2}:{TCP2_PUBLIC_PORT or TCP2_APP_PORT}",
                              "note": "همان Reality با ترنسپورت gRPC روی TCP Proxy دوم؛ ضد اختلال"})
        protocols.append({"key": "anytls", "name": "AnyTLS", "path": f"{ANYTLS_HOST or '-'}:{ANYTLS_PUBLIC_PORT or ANYTLS_APP_PORT}",
                          "core": "sing-box", "note": "پروتکل ضدتشخیص روی TLS - هسته‌ی sing-box"})
        protocols.append({"key": "shadowtls", "name": "ShadowTLS v3 + SS2022", "path": f"{STLS_HOST or '-'}:{STLS_PUBLIC_PORT or STLS_APP_PORT}",
                          "core": "sing-box", "note": "خودش را شبیه TLS واقعی نشان می‌دهد + Shadowsocks-2022 پشت آن"})
        protocols.append({"key": "hysteria2", "name": "Hysteria2", "path": f"udp:{HY2_PORT}", "core": "sing-box", "udp": True,
                          "note": "سریع‌ترین روی شبکه‌ی ناپایدار - فقط VPS (Railway UDP ندارد)"})
        protocols.append({"key": "tuic", "name": "TUIC v5", "path": f"udp:{TUIC_PORT}", "core": "sing-box", "udp": True,
                          "note": "QUIC-مبنا با کنترل ازدحام BBR - فقط VPS"})
        protocols.append({"key": "wireguard", "name": "WireGuard", "path": f"udp:{WG_PORT}", "udp": True,
                          "note": "تونل UDP مستقیم؛ مخصوص سرور شخصی/VPS - روی Railway کار نمی‌کند"})
        return {
            "host": host, "clients": out, "protocols": protocols, "settings": settings,
            "version": PANEL_VERSION,
            "wg": {"port": WG_PORT, "pub": WG["pub"], "ok": bool(WG["pub"]),
                   "enabled": proto_enabled("wireguard"), "udp_ok": udp_ok()},
            ...
            "tcp2": {"host": TCP2_HOST, "port": TCP2_PUBLIC_PORT, "app_port": TCP2_APP_PORT,
                     "service": GRPC_SERVICE, "railway": IS_RAILWAY},
            "udp": {"ok": udp_ok(), "railway": IS_RAILWAY, "hy2_port": HY2_PORT, "tuic_port": TUIC_PORT},
            "singbox": {"ok": sb_ready(), "running": sb_running(), "version": SB["ver"],
                        "note": SB.get("note", ""), "err": sb_info.get("err", ""),
                        "anytls_port": ANYTLS_APP_PORT, "stls_port": STLS_APP_PORT,
                        "anytls_host": ANYTLS_HOST, "anytls_pub_port": ANYTLS_PUBLIC_PORT,
                        "stls_host": STLS_HOST, "stls_pub_port": STLS_PUBLIC_PORT,
                        "stls_sni": STLS_SNI,
                        "anytls_on": sb_anytls_ready(), "stls_on": sb_stls_ready()},
    ...
    def do_POST(self):
        ...
        if path == "/panel/api/restore":
            ...
            for rc in raw_clients:
                ...
                wg_ip = str(rc.get("wg_ip") or "")
                if wg_ip and not re.fullmatch(r"10\.8\.\d{1,3}\.\d{1,3}", wg_ip):
                    wg_ip = ""
...
# ───────────────────────── main ─────────────────────────
def shutdown(*_):
    for p in (nginx_proc, xray_proc, sb_proc):
        try:
...
def main():
    ...
    init_reality()
    init_wg()
    provision_singbox()   # locate / auto-download the 2nd core before spawning
    start_nginx()
    restart_cores()       # Xray + sing-box with the same user set
    sampler.start()
    ...
    print(f"TCP proxy 2 (Reality-gRPC): app port {TCP2_APP_PORT}, public {TCP2_HOST or '-'}:{TCP2_PUBLIC_PORT or '-'}", flush=True)
    if sb_ready():
        print(f"sing-box {SB['ver']}: AnyTLS {ANYTLS_APP_PORT}/tcp · ShadowTLS {STLS_APP_PORT}/tcp"
              + (f" · Hysteria2 {HY2_PORT}/udp · TUIC {TUIC_PORT}/udp" if udp_ok() else " · UDP off (Railway)"), flush=True)
    else:
        print(f"sing-box: unavailable ({SB['note'] or 'not found'}) - its protocols stay off", flush=True)
    print(f"WireGuard: udp port {WG_PORT}" + ("" if udp_ok() else " (off - no public UDP on Railway)"), flush=True)
