...
[![version](https://img.shields.io/badge/version-2.3.2-d9b44a?style=flat-square&labelColor=000000)]()
...
نسخه **2.3.2** یک **هات‌فیکس حیاتی** روی 2.3.1 است: اکانت‌های اینباند Reality حالا `flow=xtls-rprx-vision` را حمل می‌کنند — نسخه‌های جدید Xray کانفیگ‌های Reality بدون flow را رد می‌کردند و کانفیگ TCP روی کلاینت Timeout می‌داد.
...
## ⚡ هات‌فیکس 2.3.2 — فیکس اتصال Reality (مهم!)

**علت:** لینک‌های Reality از همیشه `flow=xtls-rprx-vision` را درخواست می‌کردند، اما اکانت‌های سمت سرور اینباند Reality فیلد flow را نداشتند. نسخه‌های جدید Xray-core این ترکیب را رد می‌کنند و روی کلاینت فقط **Timeout** می‌بینید — در حالی که تست‌های بیرونی (باز بودن پورت، گواهی کاموفلاژ) سالم به نظر می‌رسند.

**فیکس:** اکانت‌های اینباند Reality حالا همان flow را در کانفیگ سرور دارند. لینک‌ها و کانفیگ‌های کلاینت **دست‌نخورده**‌اند — بعد از آپدیت، فقط لینک قبلی را دوباره تست کنید.

**فایل‌های تغییر یافته:** `panel.py` (یک خط در build_config + نسخه) و `panel.html` (شماره نسخه + changelog).

---

## تازه‌های نسخه 2.3.1 (تاریخچه)
...
```
nullgate-panel-v2.3.2/
├── panel.html            ← پنل وب (نسخه 2.3.2 — جایگزین فایل قبلی در ریپو)
├── panel.py              ← سرور پنل (نسخه 2.3.2 — جایگزین فایل قبلی در ریپو)
├── README.md             ← همین فایل
